...
bool ok(int q[], int c) {
    // write the ok function for 8 Queens
...
void print(int q[]) {
    ...
    int i, j, k, l;
    typedef char box[5][7];   // box is now a data type: a 5x7 2D array of char
    box bb, wb;
    box *board[8][8];
...
int main() {
    ...
    while (c >= 0) {  // NEXT COL:
        ...
        while (c >= 0) {  // NEXT ROW:
            ...
            if (q[c] > 7) c--;  // If you have passed the last row, backtrack
